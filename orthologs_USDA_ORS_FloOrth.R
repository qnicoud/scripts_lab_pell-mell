...
orth <- orth[which(orth$�..Label %in% corres$Previous.label),]
a <- corres[which(corres$Previous.label %in% orth$�..Label),]

## replace v1 ids by v2 ids
for (i in 1:dim(orth)[1]) {
    if (sum(orth$�..Label[i] %in% a$Previous.label)) {
        orth$�..Label[i] <- a$Current.label[which(a$Previous.label == orth$�..Label[i])]
    } else {
        orth$�..Label[i] <- ""
    }
}
...
orth_cln <- data.frame(Label = orth$�..Label, Bjap = gsub("\\,.*","",orth$Bradyrhizobium.japonicum.USDA.110))
...
sub_fdr <- ors$Gene.accession %in% fin_fin$ors_id
sub_fdr_filtered <- ors$AA_vs_YM.FDR[which(sub_fdr)][order(ors$Gene.accession[which(sub_fdr)] )][rank(fin_fin$ors_id)]
fdr_trim <- ors$Gene.accession[which(sub_fdr_filtered < 0.05)]
sub_ok <- which(fin_fin$ors_id %in% fdr_trim)

fin_fin <- fin_fin[sub_ok,]
...
fin_fin3 <- (fin_fin2 %>% distinct())


fin_clust <- fin_fin3[,3:4]
#fin_clust[,2] <- -fin_clust[,2]
row.names(fin_clust) <- make.names(as.character(paste(fin_fin3$ors_id, fin_fin3$usda_id, sep = "_-_")), unique =TRUE)

##Temp
row.names(fin_clust) <- make.names(as.character(paste(fin_fin3$ors_id, fin_fin3$usda_id, table$ors_annot[which(table$ors_id %in% fin_fin3$ors_id)][order(table$ors_id[which(table$ors_id %in% fin_fin3$ors_id)])][rank(fin_fin3$ors_id)], sep = "_-_")), unique =TRUE)
...
#Pour refaire les heatmap et cluster � partir de cluster_CR_all
#===========================================================#
